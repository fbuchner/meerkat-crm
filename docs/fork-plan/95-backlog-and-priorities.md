...
> one more is still pending: `i18n.IsValidLanguage` accepting any input (makes `UpdateLanguage`'s rejection
> branch unreachable), found during item 10. Tier 6 broadened from UI-only to also carry non-critical
> test-coverage expansion, both still explicitly last-priority, after everything else is ready.
> **Next up: Tier 4 (Tier 3 is fully closed).**).
