...
> 9-10 are medium test-coverage dispatches; item 11 needs its own scoping pass. Three real, unrelated bugs
> found in passing this session were spawned as separate background tasks and **all now fixed and merged**:
> the stale `dashboardsInfo` copy left over from Tier 3b (`3e88d36`), `ContactSyncLink.ETag`'s GORM
> column-name mismatch that would have broken CardDAV incremental sync writes (`ac17691`), and a handful
> of older tables (`carddav_sync`, `api_tokens`, `reminder_completions`, `calendar_subscriptions`/
...
> Tier 6 broadened from UI-only to also carry non-critical test-coverage expansion, both still explicitly
