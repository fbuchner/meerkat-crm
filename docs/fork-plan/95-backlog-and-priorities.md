...
| 12 | **Fix `i18n.IsValidLanguage` accepting any input** (found in passing during item 10's Phase 3b work; write-up only, not yet implemented) | trivial–small | **Confirmed, root-caused, not fixed.** `i18n/i18n.go:140-148`'s `IsValidLanguage` normalizes its input via `normalizeLanguage` (`i18n/i18n.go:80-95`) before checking membership in `SupportedLanguages` — but `normalizeLanguage` is designed for *display/translation-lookup* use (`T()`'s own call site), where "always return something usable" is correct: it silently falls back to `DefaultLanguage` ("en") for **any** unrecognized input, including garbage. Since `DefaultLanguage` is itself always a member of `SupportedLanguages`, `IsValidLanguage` therefore returns `true` for literally any string. Concretely, in `controllers/user_controller.go`'s `UpdateLanguage` (`:421-469`): the `!i18n.IsValidLanguage(input.Language)` rejection branch (`:431-434`) is dead code — unreachable for any input — and worse, the handler stores the **raw, unnormalized** `input.Language` directly onto `user.Language` (`:462`, not the normalized value), so a client can PUT e.g. `"language": "garbage"` and get a `200 OK` echoing `"language": "garbage"` back, with that literal string persisted to the DB. Blast radius is UX/data-hygiene, not a crash: every actual *reader* of `user.Language` (`i18n.T()`, `services/reminder_service.go:363`, `services/calendar_sync_service.go:280`) normalizes on read too, so translation lookups still silently fall back to English — but the frontend language-switcher UI likely expects `user.Language` to always be one of the 5 supported codes, and a garbage value round-tripped through `GetCurrentUser`/`ListUsers` could leave it in a broken/unselected state. **Fix sketch** (not applied): give `IsValidLanguage` its own normalization step — same first-subtag-lowercase logic `normalizeLanguage` uses (`strings.ToLower(strings.Split(lang, "-")[0])`) — but without the `DefaultLanguage` fallback, so an unrecognized code is genuinely rejected rather than silently accepted-as-English. **Open design question for whoever picks this up**: should an *empty* `input.Language` be treated as valid (meaning "reset to default/en", `normalizeLanguage`'s existing empty-string behavior) or rejected (client must always supply an explicit code)? Decide before writing the fix, since the corrected function's empty-string behavior depends on the answer. |
