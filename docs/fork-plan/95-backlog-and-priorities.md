...
> `DeleteCircle`/`DeleteTag` for free). Item 3's guard turned out already satisfied. Items 9-11 remain:
> 9-10 are medium test-coverage dispatches; item 11 needs its own scoping pass. Two real, unrelated bugs
> were found in passing and spawned as separate tasks rather than fixed inline: `ContactSyncLink.ETag`
> GORM column-name mismatch (would break CardDAV incremental sync writes, found during item 1's
> verification), and a handful of older tables (`carddav_sync`, `api_tokens`, `reminder_completions`,
> `calendar_subscriptions`/`calendar_event_links`, `relationships.related_contact_id`) that item 1's
> cascade-delete fix didn't cover and FK enforcement can't help either (found during item 8's audit). Tier
> 6 broadened from UI-only to also carry non-critical test-coverage expansion, both still explicitly
> last-priority, after everything else is ready. **Next up: Tier 3c items 9-11, or Tier 4.**).
