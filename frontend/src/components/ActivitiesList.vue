<template>
  <v-container>
    ...
    <v-timeline density="compact" side="end">
      <v-timeline-item v-for="activity in activities" :key="activity.ID" dot-color="blue lighten-3" icon="mdi-calendar">
        <div class="timeline-date-section">
          <strong>{{ formatDate(activity.date) }}</strong>
          <v-icon small class="edit-icon ml-2" @click="openEditActivity(activity)">mdi-pencil</v-icon>
          <v-icon small class="delete-icon ml-2" color="error" @click="deleteActivity(activity.ID)">mdi-delete</v-icon>

          <div>
            <h3 class="text-subtitle-1"><strong>{{ activity.title }}</strong></h3>
...
export default {
  ...
  methods: {
    async fetchActivities(page = 1) {
      try {
        const response = await activityService.getAllActivities(page, this.limit);
