<template>
  <v-container v-if="contact">
    ...
    <v-row
      class="d-flex flex-column flex-md-row align-center text-center text-md-left"
    >
      ...
      >
        <div
          class="d-flex align-center justify-center justify-md-start name-section field-label"
        >
          ...
          <template v-if="!isEditingName">
            ...
            <v-icon small class="edit-icon ml-2" @click="startEditingName"
              >mdi-pencil</v-icon
            >
            <v-icon
              small
          ...
          <template v-else>
            ...
            <v-icon small class="confirm-icon ml-2" @click="saveNameEdit"
              >mdi-check</v-icon
            >
            <v-icon small class="cancel-icon ml-2" @click="cancelNameEdit"
              >mdi-close</v-icon
            >
          </template>
        </div>
        ...
        <div>
          ...
          <v-chip-group
            v-if="contact.circles && contact.circles.length"
            column
            class="mt-2"
          >
            <v-chip
          ...
          <v-text-field
            ...
            hide-details
            class="mt-2"
            @keydown.enter="addCircle"
            @blur="showAddCircleInput = false"
          >
  ...
  computed: {
    ...
    contactFieldSchema() {
      return [
        {
          key: "nickname",
          label: this.$t("contacts.contact_fields.nickname"),
          type: "text",
        },
        {
          key: "gender",
          label: this.$t("contacts.contact_fields.gender"),
          type: "select",
          options: this.$t("contacts.contact_fields.genders").split(","),
        },
        {
          key: "birthday",
          label: this.$t("contacts.contact_fields.birthday"),
          type: "date",
          format: "DD.MM.YYYY",
        },
        {
          key: "email",
          label: this.$t("contacts.contact_fields.email"),
          type: "email",
        },
        {
          key: "phone",
          label: this.$t("contacts.contact_fields.phone"),
          type: "tel",
        },
        {
          key: "address",
          label: this.$t("contacts.contact_fields.address"),
          type: "text",
        },
        {
          key: "how_we_met",
          label: this.$t("contacts.contact_fields.how_we_met"),
          type: "textarea",
        },
        {
          key: "food_preference",
          label: this.$t("contacts.contact_fields.food_preference"),
          type: "text",
        },
        {
          key: "work_information",
          label: this.$t("contacts.contact_fields.work_information"),
          type: "text",
        },
        {
          key: "contact_information",
          label: this.$t("contacts.contact_fields.additional_information"),
          type: "textarea",
        },
      ];
    },
  ...
  methods: {
    async fetchContact() {
      try {
        ...
        if (!this.contact.reminders) {
          this.contact.reminders = [];
        }
    ...
    async addCircle() {
      ...
      try {
        ...
        await contactService.updateContact(this.ID, {
          circles: [...this.contact.circles, trimmedCircle],
        });
    ...
    async handleUpdateReminders({ action, reminder, reminderId }) {
      try {
        if (action === "add") {
          const response = await reminderService.addReminder(
            this.contact.ID,
            reminder
          );
          ...
          const updatedReminders = this.contact.reminders.filter(
            (r) => r.ID !== reminderId
          );
          this.contact.reminders = updatedReminders;
        }
      } catch (error) {
