<template>
  <v-container>
    ...
    <v-timeline density="compact" side="end">
      <v-timeline-item v-for="note in notes" :key="note.ID" dot-color="green lighten-3" icon="mdi-note-text">
        <div class="timeline-date-section">
          <strong>{{ formatDate(note.date) }}</strong>
          <v-icon small class="edit-icon ml-2" @click="openEditNote(note)">mdi-pencil</v-icon>
          <v-icon small class="delete-icon ml-2" color="error" @click="deleteNote(note.ID)">mdi-delete</v-icon>
          <p>{{ note.content }}</p>
        </div>
      </v-timeline-item>
    </v-timeline>
    ...
    <v-dialog v-model="showNoteDialog" max-width="500px" persistent>
      <noteAdd :noteId="editingNote ? editingNote.ID : null"
        :initialNote="editingNote || { content: '', date: new Date() }" @noteAdded="handleNoteAdded"
        @close="closeDialog" />
    </v-dialog>
  </v-container>
</template>
  ...
  methods: {
    async fetchNotes() {
      try {
        ...
        this.notes = (response.data || []).sort((a, b) => new Date(b.date) - new Date(a.date)); // Sort by date
      } catch (error) {
